...
class UserControllerIT {
    ...
    void setUp() {
        ...
        UserEntity userEntity = springDataUserRepository.findByEmail("test@example.com")
    ...
    void testGetUser() throws Exception {
        mockMvc.perform(get("/api/user/" + testUser.getId().value())
                        .header("Authorization", "Bearer " + jwtToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(testUser.getId().value().toString()))
                .andExpect(jsonPath("$.email").value("test@example.com"));
    }

    @Test
    void testUpdateUser() throws Exception {
        UpdateUserRequest updateUserRequest = new UpdateUserRequest("Updated Name", "updated@example.com", "newpassword");

        mockMvc.perform(put("/api/user/" + testUser.getId().value())
                        .header("Authorization", "Bearer " + jwtToken)
    ...
    void testDeleteUser() throws Exception {
        mockMvc.perform(delete("/api/user/" + testUser.getId().value())
                        .header("Authorization", "Bearer " + jwtToken))
